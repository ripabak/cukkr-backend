...
**Version:** 1.0
**Date:** April 26, 2026
**Status:** In Progress — Implementation Complete, GitHub Issues Pending
...
- [ ] **Issue created**: "Technical Enabler: barbershop_settings DB Schema & Migration"
  ...
  **Acceptance Criteria Checklist in Issue:**
  ...
  - [x] FK → `organization.id` ON DELETE CASCADE
  - [x] Migration generated: `bunx drizzle-kit generate --name add_barbershop_settings` — `drizzle/20260426125509_onboarding-barbershop-setup.sql`
...
- [ ] **Issue created**: "User Story: View Barbershop Profile (GET /api/barbershop)"
  ...
  **Implementation Tasks in Issue:**
  - [x] Task: Implement `getSettings(organizationId)` in `service.ts` — SELECT from `organization` + LEFT JOIN `barbershop_settings`
  ...
  **Tests Referenced:**
  - [x] T-01: Authenticated owner → 200 with full profile ✅
  - [x] T-02: No session → 403 ✅
...
- [ ] **Issue created**: "User Story: Update Barbershop Settings (PATCH /api/barbershop/settings)"
  ...
  **Tests Referenced:**
  - [x] T-03: Update name only → 200 ✅
  - [x] T-04: Update description and address → 200 ✅
  - [x] T-05: Update slug — available new value → 200 ✅
  - [x] T-06: Update slug — taken by another org → 409 ✅
  - [x] T-07: Update slug — own current slug → 200 ✅
  - [x] T-08: Empty body → 400 ✅
  - [x] T-09: Name too short (1 char) → 422 (Elysia TypeBox validation) ✅
  - [x] T-10: Slug invalid format (contains space) → 422 (Elysia TypeBox validation) ✅
  - [x] T-11: Slug invalid format (starts with hyphen) → 422 (Elysia TypeBox validation) ✅
  - [x] T-12: Slug too short (2 chars) → 422 (Elysia TypeBox validation) ✅
  - [x] T-13: Unauthenticated PATCH → 401 ✅
  - [x] T-18: Cross-tenant isolation — owner A cannot overwrite owner B's data ✅
  - [x] T-19: Idempotency — two identical PATCHes both return 200 ✅
...
- [ ] **Issue created**: "User Story: Real-Time Slug Availability Check (GET /api/barbershop/slug-check)"
  ...
  **Tests Referenced:**
  - [x] T-14: Available slug → `{ available: true }` ✅
  - [x] T-15: Taken slug → `{ available: false }` ✅
  - [x] T-16: Own slug while authenticated → `{ available: true }` ✅
  - [x] T-17: Missing slug param → 422 (Elysia TypeBox validation) ✅
...
- [ ] **Issue created**: "Test: Barbershop Settings Integration Tests"
  ...
  **Test File**: `tests/modules/barbershop-settings.test.ts` ✅ Created

  **Setup Tasks in Issue:**
  - [x] Sign up unique user (User A), create org, set active, capture `authCookie`
  - [x] Sign up second user (User B), create org, set active — for cross-tenant tests
  ...
  **Test Cases to Implement:**
  ...
  | T-09 | Name too short (1 char) | `PATCH /api/barbershop/settings` | 422 | ✅ |
  | T-10 | Slug with space | `PATCH /api/barbershop/settings` | 422 | ✅ |
  | T-11 | Slug starts with hyphen | `PATCH /api/barbershop/settings` | 422 | ✅ |
  | T-12 | Slug too short (2 chars) | `PATCH /api/barbershop/settings` | 422 | ✅ |
  ...
  | T-17 | Slug check — missing param | `GET /api/barbershop/slug-check` | 422 | ✅ |
